...
import { update, scheduleUpdate, willBeUpdatedSet, gridUpdateSet, gridUpdateGet } from "./engine";
...
export function renderLayout(scope = globalScope) {
  ...
  (typeof info[1] === "number" ? info[1] : parseInt(info[1] as string)),
tempBuffer.Input[i].y +
  (typeof info[2] === "number" ? info[2] : parseInt(info[2] as string)),
      12,
    );
  }
